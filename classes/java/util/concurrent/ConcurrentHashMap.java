...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
    ...
    /**
     * Spread方法主要是保证哈希值大于0，因为小于0的值，表示其他特殊用途
     */
     ...
     */
    /*
     * tabAt主要是通过Unsafe.getObjectVolatile保证取值时的可见性
     * Unsafe对象可以针对内存地址直接存取元素，并提供了保证可见性的方法
     */
    ...
    //单看setTabAt并没有线程安全上的保证，因此只能在同步块内使用
    ...
    /**
     * 如果旧节点的值为cv,则取代/删除旧节点，视value是否为null决定
     */
    final V replaceNode(Object key, V value, Object cv) {
        ...
        for (Node<K,V>[] tab = table;;) {
            Node<K,V> f; int n, i, fh;
            if (tab == null || (n = tab.length) == 0 ||
                (f = tabAt(tab, i = (n - 1) & hash)) == null) //bucket上的元素为null
                break;
            else if ((fh = f.hash) == MOVED) //如果正在迁移，则结束等继续循环时尝试
                tab = helpTransfer(tab, f);
            else {//bucket元素不为null
                V oldVal = null;
                ...
                synchronized (f) {
                    ...
                    if (tabAt(tab, i) == f) {
                            ...
                            for (Node<K,V> e = f, pred = null;;) {
                                ...
                                if (e.hash == hash &&
                                    ...
                                    if (cv == null || cv == ev ||
                                        ...
                                        if (value != null) //更新值
                                            e.val = value;
                                        else if (pred != null) //value等于null，表示删除该节点
                                            pred.next = e.next;
                                        else //value == null 且 e为第一个节点，直接将下一个节点设置到bucket上
                        ...
                        else if (f instanceof TreeBin) {//如果是树，则通过红黑书的方式去操作
                            validated = true;
    ...
    /**
     * 返回map大小，size()方法因为返回类型为int,因此大小受限
     */
    ...
    /**
     * 当一条线程在transfer时，如果另一条线程也需要transfer，则线程会加入协助扩容
     */
